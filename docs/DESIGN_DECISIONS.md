...
- **User 테이블에 Refresh Token 및 상태 필드 추가**
- **User 테이블에 보안 관련 필드 추가**: failedLoginAttempts, lockedUntil 등
- **인덱스 최적화**: 이메일, 리프레시 토큰에 인덱스 설정

**선택 이유:**
  - 캐싱 효율성 (단일 조회로 사용자 및 토큰 상태 확인) 
  - 인증 요청 시 조인 없이 단일 쿼리로 사용자 정보 및 토큰 상태 확인 가능
  - 읽기 성능 최적화로 인증 지연 최소화
  - 복잡한 쿼리 없이도 계정 상태 관리 용이

**향후 고려사항:**
- 애플리케이션 확장 시 점진적 정규화 검토
